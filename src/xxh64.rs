...
impl_core_buildhasher!(BuildHasher);
impl_core_hasher!(Xxh64);

/// `BuildHasher` for the `Xxh64` hasher.
pub struct BuildHasher(u64);
...
impl crate::BuildHasher<u64> for BuildHasher {
    type Hasher = Xxh64;
...
mod tests {
    ...
    fn empty_default_seed() {
        assert_eq!(default_seed(Empty), 0xef46db3751d8e999);
    }

    #[test]
    fn empty_custom_seed() {
        assert_eq!(custom_seed(Empty), 0x28e7a0126181c619);
    }
